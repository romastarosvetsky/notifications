...
class Notifier:
    ...
    def send_notification(self, notification_id, activation_time):
        notification_qs = Notification.objects.filter(
            pk=notification_id, sent=False, activation_time=activation_time
        ).prefetch_related('participants').select_related('owner')
        notification = notification_qs.first()
        if notification is not None:
            participants_emails_qs = notification.participants.all().values_list('email', flat=True)
            email = Email(subject=notification.title,
                          from_email=settings.EMAIL_HOST_USER,
                          to=list(participants_emails_qs),
                          email_body_context={
